...
The document has been designed to facilitate workflow and reproducibility of the data analysis. 
...
All terrestrial laser scanning data (TLS and PAVD), as well as long-term experimental data (soil, met_long, precip) of the long-term drought experiment in Caxiuanã (ESECAFLOR), will only be made available upon request.
These datasets are still cuurrently in use for other work and publications and cannot be made public yet.
...
met23 <- met23[met23$T.air42m > qq[1] & met23$T.air42m < qq[2],]

# Filter for 2023 data
ds <- met23 %>%
  filter(year == 2023)
# Extract the date from the timestamp
ds$date <- lubridate::as_date(ds$time)

ds_filt <- ds %>%
  filter(month >= 8 & month <= 11)

daily_max_min_filt <- ds_filt %>%
  group_by(date) %>%
  summarize(
    daily_max_temp = max(T.air42m, na.rm = TRUE),
    daily_max_vpd = max(vpd42m, na.rm = TRUE)*10 # convert to hPa
  ) %>%
  ungroup()

# Calculate the mean and standard error of the daily maxima and minima
# for RH (during dry season)
summary_stats <- daily_max_min_filt %>%
  summarize(
...
#ggsave("plots/sfig6.png", width = 7, height = 4)
...
met_long$date <- lubridate::as_date(met_long$time)
...
# Clean temperature data (remove unrealistic values, checked from dfsummary)
...
for (plot in unique(sap$plot)) {
  cat("Number of unique trees in plot", plot, ":", length(unique(sap$ID[sap$plot == plot])), "\n")
}
# Total unique trees
length(unique(sap$ID))
...
# predict(lmm, newdata = pred_data, re.form = NA)

# CROWN MODEL
# lmm <- lmer(resilience ~ numdate_c * plot + crown_volume_m3 + numdate_c:crown_volume_m3  + (1 | ID), data = sap.rs) # adding crown architecture and swc/biomass
lmm <- lmer(resilience ~ numdate_c * plot + log_vol_c + numdate_c:log_vol_c  + (1 | ID), data = sap.rs) # adding log crown architecture and swc/biomass
summary(lmm)
anova(lmm)
ranova(lmm, reduce.terms = TRUE)
r.squaredGLMM(lmm)
...
r.squaredGLMM(lmm)
confint(lmm)
cor(sap.rl %>% select(resilience, numdate_c, log_vol_c, swc_c), use = "complete.obs")
...
```{r Fig2, echo=FALSE, fig.show='hold', fig.width=7, fig.height=5}
...
WPmd_plot <- ggplot(data = leaf.vpd, mapping = aes(x = campaign, y = wp_md, fill = plot)) +
  ...
  geom_vline(xintercept = 3.5, linetype = 'longdash', col = '#8b8b8b', size = 0.3) +
  geom_vline(xintercept = 1.5, linetype = 'longdash', col = '#8b8b8b', size = 0.3) +
  annotate("text", x = 0.65, y = max(leaf.vpd$wp_md, na.rm = TRUE) * 0.95, label = "B)", vjust = 1, hjust = 1, size = 4, fontface = "bold")
...
WPpd_plot <- ggplot(data = leaf.vpd, mapping = aes(x = campaign, y = wp_pd, fill = plot)) +
  ...
  annotate("text", x = 0.65, y = max(leaf.vpd$wp_pd, na.rm = TRUE) * 0.95, label = "C)", vjust = 1, hjust = 1, size = 4, fontface = "bold")
...
# ggsave("plots/fig2.png", combined_wp_stringency, width = 6, height = 9, dpi = 300)
...
# Prepare TLS data for visualization
# Convert tls data to long format for plotting
plot_data <- tls %>%
  select(ID, plot, area_crown_to_dbh, volume_crown_to_dbh, volume_crown_to_dbh2) %>%
  pivot_longer(
    cols = c(area_crown_to_dbh, volume_crown_to_dbh, volume_crown_to_dbh2),
    names_to = "metric",
    values_to = "value"
  ) %>%
  ...
  mutate(
    metric_clean = case_when(      
      metric == "area_crown_to_dbh" ~ "Area:DBH\n(m² m⁻¹)",
      metric == "volume_crown_to_dbh" ~ "Volume:DBH\n(m³ m⁻¹)",
      metric == "volume_crown_to_dbh2" ~ "Volume:DBH²\n(m³ m⁻²)",
      TRUE ~ metric
    )
  )

# Create TLS metrics boxplot for top-left panel
tls_plot <- ggplot(plot_data, aes(x = treatment, y = value, fill = treatment)) +
  geom_boxplot(alpha = 0.7, outlier.alpha = 0.6, width = 0.6) +
  facet_wrap(~ metric_clean, scales = "free_y", ncol = 3) +
  scale_fill_manual(values = c("Control" = "#404788FF", "TFE" = "goldenrod3")) +
  scale_x_discrete(labels = c("Control" = "CTs", "TFE" = "PTs")) +
  labs(
    title = "A)",
    x = "",
    y = "Relative Crown Size",
    fill = "Treatment"
  ) +
  theme_bw() +
  theme(
    ...
    strip.background = element_rect(fill = "white", color = "grey50", size = 0.5),
    legend.position = "none",
...
# Create empty panel for top-right
empty_panel <- ggplot() +
  theme_void() +
...
# LAI temporal dynamics plot for bottom panel
lai_plot <- ggplot(LAI_plot_data, aes(x = date, y = Le, fill = treatment)) +
  #geom_jitter(aes(color = treatment), width = 10, alpha = 0.1, size = 2) +
  stat_summary(fun.data = "mean_cl_boot", geom = "errorbar", width = 15, 
               aes(color = treatment), size = 1.2, alpha = 0.8, linewidth = 1) +
  stat_summary(fun = "median", geom = "point", size = 3, 
               aes(color = treatment), shape = 18) +
  stat_summary(fun = "median", geom = "line", 
               aes(group = treatment, color = treatment), linewidth = 1.2, alpha = 0.9) +
  labs(
    title = "C)",
    x = "",
  ...
  theme(
    ...
    panel.border = element_rect(color = "grey50", fill = NA, linewidth = 0.8)
  ) +
  coord_cartesian(ylim = c(2.5,7)) +
  guides(fill = "none")

# Create the 3-panel layout: TLS metrics (top-left) + empty (top-right), LAI (bottom)
top_row <- tls_plot + empty_panel + plot_layout(widths = c(1, 1))
combined_figure <- top_row / lai_plot +
 plot_layout(heights = c(1, 1))

# Display the combined figure
 print(combined_figure)

#ggsave("plots/fig3.png", combined_figure, width = 8, height = 5, dpi = 300)

```
```{r PAVD forest structure and Fig4, include=FALSE}
...
pavd_plot <- ggplot(pavd_long %>% arrange(treatment, height), aes(x = PAVD_mean, y = height, color = treatment, fill = treatment)) +
 ...
 print(pavd_plot)
#ggsave("plots/fig4.png", pavd_plot, width = 8, height = 8, dpi = 300)
